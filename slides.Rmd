...
knitr::opts_chunk$set(fig.width = 13, fig.height = 4)
library(htmltools)
library(htmlwidgets)
library(mapview)
library(leaflet)
...
Also showing river habitat survey points.
...
leaflet() %>%
  ...
  addCircleMarkers(data = habitat, lat = ~Latitude, lng = ~Longitude,
                       radius = 5, color = ~pal3(HMSclass),
                       fillOpacity = 1, stroke = F, group = "Habitat Modification") %>%
  addCircleMarkers(data = habitat, lat = ~Latitude, lng = ~Longitude,
                       radius = 5, color = ~pal4(HQA),
  ...
  addEasyButton(easyButton(
    ...
    onClick = JS("function(btn, map){ map.setZoom(9);}")))%>%
  addLayersControl(baseGroups = c("Mapbox"), 
                   overlayGroups = c("Passable barriers", "Impassable barriers", "Rivers", "Habitat Quality", "Habitat Modification"), position = "topleft",
                   options = layersControlOptions(collapsed = F)) %>%
